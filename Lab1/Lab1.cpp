...
void moveImageUsingMouseWheel(WPARAM wParam) {
	if (a != 0) {
		...
		{
			(imageCoorg.x - (a / 10) > clientRect.left) && (imageCoorg.x - (a / 10) <= clientRect.right - BITMAP_SIZE) ? (a -= a >= 0 ? 1 : -1) : a = -a;
			imageCoorg.x -= a / 10;
		}
		else
		{
			(imageCoorg.y + (a / 10) >= clientRect.top) && (imageCoorg.y + (a / 10) < clientRect.bottom - BITMAP_SIZE) ? a -= a >= 0 ? 1 : -1 : a = -a;
			imageCoorg.y += a / 10;
		}
	}
}
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		hBmp = LoadImage(NULL, L"penguin.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_LOADTRANSPARENT);
	...
	case WM_MOUSEWHEEL:
	{
		if (isMovingUsingMouse)
			break;
		if (a==0)
			a =(GET_WHEEL_DELTA_WPARAM(wParam) / 10) * 10;
